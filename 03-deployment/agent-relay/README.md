...
The dashboard accepts one or more agent tokens, one per line. Supplying the
tokens for both sides of an exchange combines their authorized task views and
delivery history; it does not make task data public.

## Run with PostgreSQL

The deployment stack uses PostgreSQL instead of the SQLite fallback:

```bash
docker compose up --build -d
```

The Compose file starts the database service as `postgres` and the relay at
<http://127.0.0.1:8001/>. PostgreSQL data is persisted in the named
`postgres-data` volume. Set `RELAY_DATABASE_URL` to another PostgreSQL URL for
