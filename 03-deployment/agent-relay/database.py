...
SQLite remains available for the local test suite, while the Compose
deployment uses PostgreSQL. The transaction helper keeps storage independent
of the selected database: SQLite uses ``BEGIN IMMEDIATE`` and PostgreSQL uses
a normal transaction with row locks at claim time.
"""
...
def immediate_transaction() -> Generator[Session, None, None]:
    """Run an atomic writer transaction for claims and terminal updates.

    SQLite reserves the writer slot with ``BEGIN IMMEDIATE``. PostgreSQL uses
    a regular transaction; claim selection adds ``FOR UPDATE SKIP LOCKED`` in
    :func:`storage.claim_one`.
    """
